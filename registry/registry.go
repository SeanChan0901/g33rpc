...
func (r *G33Registry) aliveServers() []string {
	...
	for addr, s := range r.servers {
		if r.timeout == 0 || s.start.Add(r.timeout).After(time.Now()) {
			alive = append(alive, addr)  // not timeout yet
		} else {
...
func (r *G33Registry) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	case "POST":
		// keep it simple, server is in req.Header
		addr := req.Header.Get("X-G33rpc-Servers")
		if addr == "" {
...
// HandleHTTP registers an HTTP handler for G33Registry messages on registryPath
...
func sendHeartbeat(registry, addr string) error {
	...
	req.Header.Set("x_G33rpc-Servers", addr)
